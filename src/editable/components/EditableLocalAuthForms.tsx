...
const inputClass = 'h-[3.5rem] rounded-[1.2rem] border border-[color:var(--slot4-card-line)] bg-[var(--slot4-surface-bg)] px-4 text-base font-semibold text-[var(--slot4-highlight-strong)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent-fill)]'
const buttonClass = 'inline-flex h-[3.5rem] items-center justify-center rounded-full bg-[#c03679] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:-translate-y-0.5 hover:bg-[var(--slot4-accent-fill)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.2rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
